  ...
  return (
    ...
    <div className="w-full h-full -z-1 absolute inset-0"> {/* Adjusted class name */}
      {heroElements.map((element, index) => (
        <div
          key={index}
          className={`absolute w-full h-full ${index === currentElement ? "visible" : "hidden"
            }`}
        >
          <div
            className={`video-slide w-full h-full video-element z-[-1] ${index === currentElement ? "video-fade-in" : "video-fade-out"
              }`}
          >
            <img
              src={element.img}
          ...
          <div
            className={`text-slide h-[40%] flex flex-col items-center justify-center absolute bottom-7 w-[80%] mr-auto ${index === currentElement ? "text-fade-in" : "text-fade-out"
              }`}
          >
